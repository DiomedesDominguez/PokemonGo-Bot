namespace PokemonGo.RocketAPI.Logic
{
    ...
    public class Logic
    {
        ...
        public static int failed_softban;
        ...
        {
            var pokemonToEvolve = (await _client.Inventory.GetPokemonToEvolve(filter)).ToList();
            ...
            foreach (var pokemon in pokemonToEvolve)
            {
                ...
                if (evolvePokemonOutProto.Result == EvolvePokemonResponse.Types.Result.Success)
                {
                    Logger.ColoredConsoleWrite(ConsoleColor.Green,
                        $"Evolved {StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId)} CP {pokemon.Cp} {PokemonInfo.CalculatePokemonPerfection(pokemon).ToString("0.00")}%  to {StringUtils.getPokemonNameByLanguage(_clientSettings, evolvePokemonOutProto.EvolvedPokemonData.PokemonId)} CP: {evolvePokemonOutProto.EvolvedPokemonData.Cp} for {evolvePokemonOutProto.ExperienceAwarded.ToString("N0")}xp",
                        LogLevel.Info);
                    _botStats.AddExperience(evolvePokemonOutProto.ExperienceAwarded);

                    try
                    {
                        TelegramUtil.getInstance()
                            .sendInformationText(TelegramUtil.TelegramUtilInformationTopics.Evolve,
                                StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId), pokemon.Cp,
                                PokemonInfo.CalculatePokemonPerfection(pokemon).ToString("0.00"),
                                StringUtils.getPokemonNameByLanguage(_clientSettings,
                                    evolvePokemonOutProto.EvolvedPokemonData.PokemonId),
                                evolvePokemonOutProto.EvolvedPokemonData.Cp,
                                evolvePokemonOutProto.ExperienceAwarded.ToString("N0"));
                    }
                    catch (Exception)
                    {
                    }
                ...
                else
                {
                    if (evolvePokemonOutProto.Result != EvolvePokemonResponse.Types.Result.Success)
                    {
                        Logger.ColoredConsoleWrite(ConsoleColor.Red,
                            $"Failed to evolve {pokemon.PokemonId}. EvolvePokemonOutProto.Result was {evolvePokemonOutProto.Result}",
                            LogLevel.Info);
                    }
                }
        ...
        public async Task Execute()
        {
            ...
            Logger.ColoredConsoleWrite(ConsoleColor.Green,
                $"Starting Execute on login server: {_clientSettings.AuthType}", LogLevel.Info);
            ...
            while (true)
            {
                try
                {
                    ...
                    if (!string.IsNullOrEmpty(_clientSettings.TelegramAPIToken) &&
                        !string.IsNullOrEmpty(_clientSettings.TelegramName))
                    {
                        try
                        {
                            _telegram = new TelegramUtil(_client,
                                new TelegramBotClient(_clientSettings.TelegramAPIToken), _clientSettings,
                                _client.Inventory);

                            Logger.ColoredConsoleWrite(ConsoleColor.Green,
                                "To activate informations with Telegram, write the bot a message for more informations");
        ...
        private async Task ExecuteCatchAllNearbyPokemons()
        {
            ...
            foreach (var pokemon in pokemons)
            {
                ...
                if (count >= 6)
                {
                    ...
                    await
                        TransferDuplicatePokemon(_clientSettings.keepPokemonsThatCanEvolve,
                            _clientSettings.TransferFirstLowIV);
                    //await RecycleItems();
                }
                ...
                if (encounterPokemonResponse?.Status == EncounterResponse.Types.Status.EncounterSuccess)
                {
                    ...
                    {
                        Logger.ColoredConsoleWrite(ConsoleColor.Red,
                            $"No Pokeballs! - missed {pokemon.PokemonId} CP {encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp} IV {PokemonInfo.CalculatePokemonPerfection(encounterPokemonResponse.WildPokemon.PokemonData).ToString("0.00")}%");

                        return;
                    }

                    var inventoryBerries = (await _client.Inventory.GetItems()).ToList();
                    ...
                    Logger.ColoredConsoleWrite(ConsoleColor.Magenta,
                        $"Encountered {StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId)} CP {encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp}  IV {PokemonInfo.CalculatePokemonPerfection(encounterPokemonResponse.WildPokemon.PokemonData).ToString("0.00")}% Probability {Math.Round(probability.Value * 100)}%");
                    do
                    {
                        if (probability.HasValue && probability.Value < _clientSettings.razzberry_chance &&
                            _clientSettings.UseRazzBerry)
                        {
                            var bestBerry = await GetBestBerry(encounterPokemonResponse?.WildPokemon);
                            var berries = inventoryBerries.FirstOrDefault(p => p.ItemId == bestBerry);
                            if (bestBerry != ItemId.ItemUnknown)
                            {
                                //Throw berry if we can
                                var useRaspberry =
                                    await
                                        _client.Encounter.UseCaptureItem(pokemon.EncounterId, bestBerry,
                                            pokemon.SpawnPointId);
                                Logger.ColoredConsoleWrite(ConsoleColor.Green,
                                    $"Thrown {bestBerry}. Remaining: {berries.Count}.", LogLevel.Info);
                        ...
                        caughtPokemonResponse =
                            await
                                _client.Encounter.CatchPokemon(pokemon.EncounterId, pokemon.SpawnPointId, bestPokeball);
                        switch (caughtPokemonResponse.Status)
                        {
                            case CatchPokemonResponse.Types.CatchStatus.CatchMissed:
                                Logger.ColoredConsoleWrite(ConsoleColor.Magenta,
                                    $"Missed {StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId)} while using {bestPokeball}");
                                break;
                            case CatchPokemonResponse.Types.CatchStatus.CatchEscape:
                                Logger.ColoredConsoleWrite(ConsoleColor.Magenta,
                                    $"{StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId)} escaped while using {bestPokeball}");
                                break;
                        }
                    ...
                    if (caughtPokemonResponse.Status == CatchPokemonResponse.Types.CatchStatus.CatchSuccess)
                    {
                        ...
                        _infoObservable.PushNewHuntStats(
                            $"{pokemon.Latitude}/{pokemon.Longitude};{pokemon.PokemonId};{curDate.Ticks};{curDate}" + Environment.NewLine);
                        ...
                        {
                            TelegramUtil.getInstance()
                                .sendInformationText(TelegramUtil.TelegramUtilInformationTopics.Catch,
                                    StringUtils.getPokemonNameByLanguage(_clientSettings, pokemon.PokemonId),
                                    encounterPokemonResponse?.WildPokemon?.PokemonData?.Cp,
                                    PokemonInfo.CalculatePokemonPerfection(
                                        encounterPokemonResponse.WildPokemon.PokemonData).ToString("0.00"), bestPokeball,
                ...
                {
                    Logger.ColoredConsoleWrite(ConsoleColor.Red,
                        $"Error catching Pokemon: {encounterPokemonResponse?.Status}");
                }
        ...
        {
            var distanceFromStart = LocationUtils.CalculateDistanceInMeters(_clientSettings.DefaultLatitude,
            ...
            if (_clientSettings.MaxWalkingRadiusInMeters != 0 &&
                distanceFromStart > _clientSettings.MaxWalkingRadiusInMeters)
            {
                Logger.ColoredConsoleWrite(ConsoleColor.Green,
                    "You're outside of the defined max. walking radius. Walking back!");
                var update =
                    await
                        ...
                        .OrderBy(
                            i =>
                                LocationUtils.CalculateDistanceInMeters(_client.CurrentLatitude,
                                    _client.CurrentLongitude, i.Latitude, i.Longitude)).ToArray(),
            ...
            if (_clientSettings.MaxWalkingRadiusInMeters != 0)
            {
                pokeStops =
                    pokeStops.Where(
                        i =>
                            LocationUtils.CalculateDistanceInMeters(_client.CurrentLatitude, _client.CurrentLongitude,
                                i.Latitude, i.Longitude) <= _clientSettings.MaxWalkingRadiusInMeters).ToArray();
                if (!pokeStops.Any())
                {
                    Logger.ColoredConsoleWrite(ConsoleColor.Red,
                        "We can't find any PokeStops in a range of " + _clientSettings.MaxWalkingRadiusInMeters + "m!");
            ...
            {
                Logger.ColoredConsoleWrite(ConsoleColor.Red,
                    "We can't find any PokeStops, which are unused! Probably the server are unstable, or you visted them all. Retrying..");
                await ExecuteCatchAllNearbyPokemons();
            }
            else
            {
                Logger.ColoredConsoleWrite(ConsoleColor.Yellow,
                    "We found " + pokeStops.Count() + " usable PokeStops near your current location.");
            }
            ...
            foreach (var pokeStop in pokeStops)
            {
                ...
                Logger.ColoredConsoleWrite(ConsoleColor.Green,
                    $"Next Pokestop: {fortInfo.Name} in {distance:0.##}m distance.");
                var update =
                    await
                ...
                if (fortSearch.ExperienceAwarded > 0)
                {
                    ...
                    Logger.ColoredConsoleWrite(ConsoleColor.Green,
                        $"Farmed XP: {fortSearch.ExperienceAwarded}{gems}{egg}, Items: {items}", LogLevel.Info);
                    ...
                    {
                        TelegramUtil.getInstance()
                            .sendInformationText(TelegramUtil.TelegramUtilInformationTopics.Pokestop, fortInfo.Name,
                                fortSearch.ExperienceAwarded, eggs, fortSearch.GemsAwarded, items);
                    }
                    catch (Exception)
                    {
                    }
        ...
        private async Task<ItemId> GetBestBall(WildPokemon pokemon)
        {
            ...
            var balls = items.Where(i => (i.ItemId == ItemId.ItemPokeBall
                                          ...
                                          || i.ItemId == ItemId.ItemMasterBall) && i.ItemId > 0)
                .GroupBy(i => i.ItemId)
                .ToList();
            if (!balls.Any())
            {
        ...
        private async Task StartIncubation()
        {
            try
            {
                ...
                var unusedEggs =
                    (await _client.Inventory.GetEggs()).Where(x => string.IsNullOrEmpty(x.EggIncubatorId))
                ...
                var rememberedIncubatorsFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory + "\\Configs",
                    "incubators.json");
                ...
                foreach (var incubator in rememberedIncubators)
                {
                    ...
                    Logger.ColoredConsoleWrite(ConsoleColor.DarkYellow,
                        "Egg hatched and we got a " + hatched.PokemonId + " CP: " + hatched.Cp + " MaxCP: " +
                        PokemonInfo.CalculateMaxCP(hatched) + " Level: " + PokemonInfo.GetLevel(hatched) + " IV: " +
                        PokemonInfo.CalculatePokemonPerfection(hatched).ToString("0.00") + "%");
                }
                ...
                foreach (var incubator in incubators)
                {
                    if (incubator.PokemonId == 0)
                    {
                        ...
                        Logger.ColoredConsoleWrite(ConsoleColor.DarkYellow,
                            "Added Egg which needs " + egg.EggKmWalkedTarget + "km");
                    ...
                    else
                    {
                        ...
                        Logger.ColoredConsoleWrite(ConsoleColor.DarkYellow,
                            "Egg (" + (incubator.TargetKmWalked - incubator.StartKmWalked) + "km) need to walk " +
                            Math.Round(incubator.TargetKmWalked - kmWalked, 2) + " km.");
                    }
                }
        ...
        private async Task StatsLog(Client client)
        {
            ...
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                "-----------------------[PLAYER STATS UPDATE]-----------------------");
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                $"Level/EXP: {stats.Level} | {curexp.ToString("N0")}/{expneeded.ToString("N0")} ({Math.Round(curexppercent, 2)}%) | EXP to Level up: " +
                (stats.NextLevelXp - stats.Experience));
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                "PokeStops visited: " + stats.PokeStopVisits + " | KM Walked: " + Math.Round(stats.KmWalked, 2));
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                "Pokemon: " + await _client.Inventory.getPokemonCount() + " + " + await _client.Inventory.GetEggsCount() +
                " Eggs /" + profile.PlayerData.MaxPokemonStorage + " (" + pokemonToEvolve + " Evolvable)");
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                "Pokedex Completion: " + stats.UniquePokedexEntries + "/150 " + "[" + pokedexpercent + "%]");
            Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                "Items: " + await _client.Inventory.getInventoryCount() + "/" + profile.PlayerData.MaxItemStorage +
                " | Stardust: " + profile.PlayerData.Currencies.ToArray()[1].Amount.ToString("N0"));
            ...
            else if (stats.Level > level)
            {
                ...
                foreach (var i in lvlup.ItemsAwarded)
                {
                    ...
                    {
                        Logger.ColoredConsoleWrite(ConsoleColor.Magenta,
                            "Got Item: " + i.ItemId + " " + i.ItemCount + "x");
            ...
            Console.Title = profile.PlayerData.Username + " lvl" + stats.Level + "-(" +
                            (stats.Experience - stats.PrevLevelXp -
                             StringUtils.getExpDiff(stats.Level)).ToString("N0") + "/" +
                            (stats.NextLevelXp - stats.PrevLevelXp - StringUtils.getExpDiff(stats.Level)).ToString("N0") +
                            "|" + Math.Round(curexppercent, 2) + "%)| Stardust: " +
                            profile.PlayerData.Currencies.ToArray()[1].Amount + "| " + _botStats;
        }

        private async Task TransferDuplicatePokemon(bool keepPokemonsThatCanEvolve = false,
            bool TransferFirstLowIV = false)
        {
            if (_clientSettings.TransferDoublePokemons)
            {
                var duplicatePokemons =
                    await _client.Inventory.GetDuplicatePokemonToTransfer(keepPokemonsThatCanEvolve, TransferFirstLowIV);
                ...
                foreach (var duplicatePokemon in duplicatePokemons)
                {
                    if (!_clientSettings.pokemonsToHold.Contains(duplicatePokemon.PokemonId))
                    {
                        if (duplicatePokemon.Cp >= _clientSettings.DontTransferWithCPOver ||
                            PokemonInfo.CalculatePokemonPerfection(duplicatePokemon) >= _client.Settings.ivmaxpercent)
                        {
                            continue;
                        }
                        ...
                        {
                            Logger.ColoredConsoleWrite(ConsoleColor.Yellow,
                                $"Transfer {StringUtils.getPokemonNameByLanguage(_clientSettings, duplicatePokemon.PokemonId)} CP {duplicatePokemon.Cp} IV {PokemonInfo.CalculatePokemonPerfection(duplicatePokemon).ToString("0.00")} % (Best IV: {PokemonInfo.CalculatePokemonPerfection(bestPokemonsIVOfType.First()).ToString("0.00")} %)",
                                LogLevel.Info);
                        }
                        else
                        {
                            Logger.ColoredConsoleWrite(ConsoleColor.Yellow,
                                $"Transfer {StringUtils.getPokemonNameByLanguage(_clientSettings, duplicatePokemon.PokemonId)} CP {duplicatePokemon.Cp} IV {PokemonInfo.CalculatePokemonPerfection(duplicatePokemon).ToString("0.00")} % (Best: {bestPokemonsCPOfType.First().Cp} CP)",
                                LogLevel.Info);
                        }
                        ...
                        {
                            TelegramUtil.getInstance()
                                .sendInformationText(TelegramUtil.TelegramUtilInformationTopics.Transfer,
                                    StringUtils.getPokemonNameByLanguage(_clientSettings, duplicatePokemon.PokemonId),
                                    duplicatePokemon.Cp,
                                    PokemonInfo.CalculatePokemonPerfection(duplicatePokemon).ToString("0.00"),
                                    bestPokemonOfType);
                        }
                        catch (Exception)
                        {
        ...
        public async Task UseIncense()
        {
            if (_clientSettings.UseIncense)
            {
                ...
                if (lastincenseuse > DateTime.Now.AddSeconds(5))
                {
                    ...
                    Logger.ColoredConsoleWrite(ConsoleColor.Cyan,
                        $"Incense still running: {duration.Minutes}m{duration.Seconds}s");
                    return;
                }
        ...
        #region Nested type: IncubatorUsage
        ...
        private class IncubatorUsage : IEquatable<IncubatorUsage>
        {
            ...
            #region IEquatable<IncubatorUsage> Members
